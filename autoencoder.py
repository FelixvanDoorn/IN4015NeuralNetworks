...
if __name__ == "__main__":
  ...
  flags.DEFINE_string(
      "train_data_pattern", "C:/Users/Philip/Desktop/youtubeKaggleCompetition/frameLevelData/train*.tfrecord",
...
def build_graph(reader,
  ...
  predictions_autoencoder = tf.get_collection("predictions_autencoder")[0]
  model_input_raw = predictions_autoencoder
...
def build_graph_autoencoder(reader,
                model,
                train_data_pattern,
                label_loss_fn=losses.CrossEntropyLoss(),
                batch_size=1000,
  ...
  tf.add_to_collection("global_step", global_step)
  tf.add_to_collection("loss", label_loss)
  tf.add_to_collection("predictions_autoencoder", tf.concat(tower_predictions, 0))
  tf.add_to_collection("input_batch_raw", model_input_raw)
  tf.add_to_collection("input_batch", model_input)
  tf.add_to_collection("num_frames", num_frames)
  tf.add_to_collection("labels", tf.cast(labels_batch, tf.float32))
...
#                           task_as_string(self.task))
    ...
    with tf.Graph().as_default() as graph:
      ...
      with tf.device(device_fn):
        if not meta_filename:
          saver = self.build_model_autoencoder(self.model, self.reader)
      ...
      try:
        ...
        while (not sv.should_stop()) and (not self.max_steps_reached):
          ...
          _, global_step_val, loss_val, predictions_val, labels_val = sess.run(
              [train_op, global_step, loss, predictions, input_batch])
  ...
  def build_model_autoencoder(self, model, reader):
    """Find the model and build the graph."""

    label_loss_fn = find_class_by_name(FLAGS.label_loss, [losses])()
    optimizer_class = find_class_by_name(FLAGS.optimizer, [tf.train])

    build_graph_autoencoder(reader=reader,
                 model=model,
                 optimizer_class=optimizer_class,
                 clip_gradient_norm=FLAGS.clip_gradient_norm,
                 train_data_pattern=FLAGS.train_data_pattern,
