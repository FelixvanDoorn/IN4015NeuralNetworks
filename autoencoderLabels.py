# -*- coding: utf-8 -*-
"""
Created on Sun May 21 20:45:51 2017
...
def convert(label):
    split_label = map(int,label.split(" "))
    output = np.zeros(4716)
    for i in range(len(split_label)):
        output[split_label[i]] = 1
    return output


maxData = 1000000 # change this to maximum possible value
...
a= sess.run(h, feed_dict={x: input_data})
print(a.shape)
